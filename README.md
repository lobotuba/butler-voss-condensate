...
| `test_deconfinement.py` | **Deconfining the spin-2 sector — `+R` → `−1/r`, given `μ>0`.** The confining sector is the **biharmonic** `κ∇⁴` (propagator `1/q⁴` → constant force = a string tension `s²/8πκ`). The Sakharov-induced Einstein term adds `μ∇²`; the full propagator `1/(κq⁴+μq²)` has the exact closed form `G(r)=(1/4πμr)(1−e^{−r/ℓ})`, `ℓ=√(κ/μ)`. Measured (3D **Dirichlet**-wall sine-transform solve — periodic FFT's Ewald background distorts the tail): μ=0 force is a flat plateau at the tension (ratio **0.95**); μ>0 force matches the closed form to **<1%** (box-gated `1.8→1.3→0.7%`), turning over at `ℓ` into a **`1/r²` Newton tail**, `G=1/(4πμ)`. So the lower-derivative induced term dominates the IR and **deconfines** the graviton. Honest scope: this assumes the induced `μ>0`; its **sign** (Sakharov `μ~N_f Λ²>0`) is quoted, not measured — now the whole ballgame for tensor gravity |
